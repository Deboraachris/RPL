...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['gambar']['error'] == 0) {
        ...
        $stmt = $conn->prepare("UPDATE berita SET judul=?, deskripsi=?, foto=?, tanggal=NOW() WHERE id=?");
        $stmt->bind_param("sssi", $judul, $deskripsi, $gambar, $id);
    } else {
        $stmt = $conn->prepare("UPDATE berita SET judul=?, deskripsi=?, tanggal=NOW() WHERE id=?");
  ...
  <div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow">
    ...
    <form method="POST" enctype="multipart/form-data">
      <div class="mb-4">
        ...
        <input type="text" name="judul" value="<?= htmlspecialchars($berita['judul']); ?>" required class="w-full border px-3 py-2 rounded" />
      </div>

      <div class="mb-4">
        <label class="block font-semibold mb-1">Deskripsi</label>
        <textarea name="deskripsi" rows="5" required class="w-full border px-3 py-2 rounded"><?= htmlspecialchars($berita['deskripsi']); ?></textarea>
      </div>
      ...
      <div class="mb-4">
        ...
        <img src="tampil-gambar-berita.php?id=<?= $berita['id']; ?>" alt="Gambar Berita" class="w-32 h-32 object-cover rounded border" />
      </div>

      <div class="flex justify-between mt-6">
        <a href="berita.php" class="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded">Batal</a>
